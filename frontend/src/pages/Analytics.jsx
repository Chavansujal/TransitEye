  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-6 border-cyan-500/25">
        <div>
          <div className="flex items-center gap-2 text-cyan-400 font-mono text-xs mb-1">
            <BarChart3 className="w-3.5 h-3.5" /> SMART CITY URBAN INTELLIGENCE ANALYTICS
          </div>
      ...
      <div className="glass-panel p-6 space-y-4 border-slate-800">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <Navigation className="w-5 h-5 text-cyan-400" /> Origin-Destination (OD) Route Delay Analysis
          </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {odAnalysis.map((od, idx) => (
            <div key={idx} className="bg-slate-950/70 p-4 rounded-2xl border border-slate-800 space-y-2.5 shadow-md">
              <div className="flex items-center justify-between">
                <span className="text-xs font-bold text-cyan-400 font-mono">{od.route}</span>
                <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-mono font-extrabold ${
              ...
              <div className="flex items-center justify-between text-sm text-white font-bold font-sans">
                <span>{od.origin}</span>
                <span className="text-cyan-400 font-mono text-xs">➔</span>
                <span>{od.destination}</span>
              </div>
              <div className="flex items-center justify-between text-xs font-mono pt-1">
                <span className="text-slate-400">Average Transit Delay:</span>
                <span className="text-rose-400 font-bold text-sm">+{od.avgDelayMin} mins</span>
              </div>
              <p className="text-[11px] text-slate-400 italic">Primary Bottleneck Cause: {od.cause}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-panel p-6 space-y-4 border-slate-800">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <TrendingUp className="w-5 h-5 text-emerald-400" /> Hourly Traffic Congestion Trend
          </h3>
          <div className="h-64">
        ...
        <div className="glass-panel p-6 space-y-4 border-slate-800">
          <h3 className="text-base font-bold text-white flex items-center gap-2">
            <BarChart3 className="w-5 h-5 text-amber-400" /> Edge Events Distribution by Category
          </h3>
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
