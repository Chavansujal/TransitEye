...
export default function IncidentCenter({ incidents = [], onIncidentUpdated }) {
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6 max-w-7xl mx-auto">
      {/* Title Header */}
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 glass-panel p-4 sm:p-6 border-purple-500/25">
        <div>
          <div className="flex items-center gap-2 text-purple-400 font-mono text-[10px] sm:text-xs mb-1">
            <ShieldAlert className="w-3.5 h-3.5 shrink-0" /> AUTOMATED INCIDENT TRIAGE & ANPR IDENTIFICATION
          </div>
        ...
        {/* Security Tag */}
        <div className="flex items-center gap-2 bg-slate-950 p-2.5 sm:p-3 rounded-2xl border border-emerald-800 text-[11px] sm:text-xs font-mono text-emerald-400 shadow-lg self-start md:self-auto">
      ...
      <div className="grid grid-cols-1 gap-4 sm:gap-6">
        {incidents.map((inc) => (
          <div 
            key={inc.id} 
            className="glass-panel p-4 sm:p-6 border-slate-800 hover:border-slate-700 transition flex flex-col lg:flex-row gap-4 sm:gap-6 shadow-xl"
          >
            ...
            <div className="w-full lg:w-80 rounded-2xl bg-slate-950 border border-slate-800 overflow-hidden relative group shrink-0 shadow-inner flex flex-col justify-between">
              <div className="h-48 sm:h-52 relative overflow-hidden bg-slate-950">
                ...
                <div className="absolute top-2.5 left-2.5 bg-slate-950/90 px-2 sm:px-2.5 py-1 rounded-lg text-[9px] sm:text-[10px] font-mono text-cyan-400 border border-cyan-800 flex items-center gap-1.5 shadow-md">
                  <Camera className="w-3 h-3 shrink-0" /> {activeVideoId === inc.id ? "360° FEED" : "SNAPSHOT"}
                </div>

                {/* Toggle Video/Snapshot Button */}
                <button
                  onClick={() => setActiveVideoId(activeVideoId === inc.id ? null : inc.id)}
                  className="absolute top-2.5 right-2.5 bg-slate-950/90 hover:bg-slate-900 px-2 py-1 rounded-lg text-[9px] sm:text-[10px] font-mono text-amber-300 border border-amber-500/50 flex items-center gap-1 shadow-md transition z-10"
                >
                  <Film className="w-3 h-3 text-amber-400 shrink-0" />
                  <span>{activeVideoId === inc.id ? "SNAPSHOT" : "360° VIDEO"}</span>
                </button>

                <div className="absolute bottom-2.5 left-2.5 right-2.5 bg-slate-950/95 p-1.5 sm:p-2 rounded-xl text-[11px] sm:text-xs font-mono text-amber-300 border border-amber-500/40 text-center font-bold shadow-2xl">
                  PLATE: {inc.registrationNumber || "MH12 AB 1234"} ({((inc.anprConfidence || 0.91) * 100).toFixed(0)}% Match)
                </div>
              </div>

              {/* Secure Transmission Integrity Block */}
              <div className="p-2.5 sm:p-3 bg-slate-900/90 border-t border-slate-800 text-[10px] font-mono text-slate-400 space-y-1">
            ...
            <div className="flex-1 flex flex-col justify-between space-y-4 min-w-0">
              <div className="space-y-3">
                <div className="flex flex-wrap items-center justify-between gap-2.5 border-b border-slate-800 pb-3">
                  <div className="flex items-center gap-2.5 flex-wrap">
                    <span className="text-base sm:text-lg font-extrabold text-white">{inc.type}</span>
                  ...
                  {/* Status Badge */}
                  <span className={`px-3 py-1 rounded-full text-[11px] sm:text-xs font-mono font-bold ${
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 font-mono text-[11px] sm:text-xs text-slate-400">
                  <div>ID: <strong className="text-cyan-400 font-bold">{inc.id}</strong></div>
                  <div>Bus: <strong className="text-white font-bold">{inc.busId}</strong></div>
                ...
                <p className="text-xs text-slate-300 bg-slate-950/70 p-3 sm:p-3.5 rounded-xl border border-slate-800 font-sans leading-relaxed">
                  {inc.details}
                </p>
                ...
                <div className="bg-slate-950/80 p-3 rounded-xl border border-slate-800 font-mono text-xs space-y-2">
                  <div className="text-[10px] text-cyan-400 uppercase tracking-wider font-bold flex items-center gap-1.5">
                    <Route className="w-3.5 h-3.5 shrink-0" /> Offending Vehicle Tracking Timeline
                  </div>
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 text-[10px] sm:text-[11px]">
                    <div className="p-2 rounded-lg bg-slate-900 border border-slate-800 space-y-0.5">
                      <span className="text-slate-400 text-[9px] block">T0 • Initial Detection:</span>
                      <span className="text-white font-bold">Speed: 82 km/h</span>
                      <span className="text-slate-400 text-[9px] block">Rear Cam Approach</span>
                    </div>
                    <div className="p-2 rounded-lg bg-slate-900 border border-rose-900/60 space-y-0.5">
                      <span className="text-rose-400 text-[9px] block">T1 • Dangerous Maneuver:</span>
                      <span className="text-rose-300 font-bold">Erratic Weaving (85 km/h)</span>
                      <span className="text-slate-400 text-[9px] block">Side Cam Tagged</span>
                    </div>
                    <div className="p-2 rounded-lg bg-slate-900 border border-emerald-900/60 space-y-0.5">
                      <span className="text-emerald-400 text-[9px] block">T2 • ANPR Plate Locked:</span>
                      <span className="text-emerald-300 font-bold">MH12 AB 1234 (91%)</span>
                      <span className="text-slate-400 text-[9px] block">Front Cam Extraction</span>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 pt-3 border-t border-slate-800">
                <div className="text-xs text-slate-400 flex items-center gap-1.5 font-mono">
                  <MapPin className="w-4 h-4 text-cyan-400 shrink-0" /> Location: <span className="text-white font-bold">{inc.location}</span>
                </div>

                <div className="flex flex-wrap items-center gap-1.5 sm:gap-2 font-mono w-full sm:w-auto">
                  {["NEW", "UNDER REVIEW", "DISPATCHED", "RESOLVED"].map((st) => (
                    <button
                      key={st}
                      disabled={updatingId === inc.id || inc.status === st}
                      onClick={() => handleStatusChange(inc.id, st)}
                      className={`px-2.5 sm:px-3 py-1 sm:py-1.5 rounded-xl text-[11px] sm:text-xs font-semibold transition duration-200 ${
                        inc.status === st
