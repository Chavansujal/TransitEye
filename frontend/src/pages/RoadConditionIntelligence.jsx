  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-stretch md:items-center justify-between gap-4">
        {/* Category Filters */}
        <div className="flex flex-wrap items-center gap-1.5 bg-slate-950/80 p-1.5 rounded-2xl border border-slate-800 text-xs font-mono">
        ...
        <div className="glass-panel px-4 py-2 bg-slate-950/80 border-slate-800 text-xs font-mono flex items-center gap-2 text-slate-400">
          <Shield className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
          <span>Priority = Severity × Frequency × Bus Corroborations</span>
        </div>
      </div>
            ...
            >
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800/80 pb-4">
                <div className="space-y-1.5">
                  <div className="flex flex-wrap items-center gap-3">
                    ...
                    {isDispatched && (
                      <span className="bg-emerald-950 text-emerald-300 border border-emerald-700 px-2.5 py-0.5 rounded-full text-[10px] font-mono font-bold flex items-center gap-1">
                ...
                {/* Priority Score Gauge & Action Button */}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-1">
                  <div className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider">TOTAL BUS SIGHTINGS</div>
                  <div className="text-xl font-bold text-white font-mono">{issue.reports} Passes Logged</div>
                </div>

                <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-1">
                  <div className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider">INDEPENDENT BUS CORROBORATION</div>
                ...
                <div className="bg-slate-950/60 p-4 rounded-2xl border border-slate-800 space-y-1">
                  <div className="text-[10px] text-slate-400 font-mono font-bold uppercase tracking-wider">MUNICIPAL SLA / STATUS</div>
                  <div className={`text-sm font-bold font-mono ${isDispatched ? "text-emerald-400" : "text-amber-400"}`}>
                    {isDispatched ? "DISPATCHED TO ROAD CREW" : issue.status}
                  </div>
                </div>
              </div>

              {/* Municipal Action Recommendation & Dispatch Button */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 p-4 bg-cyan-950/40 border border-cyan-800/60 rounded-2xl text-xs">
                ...
                <button
                  ...
                  className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-cyan-500 to-blue-600 hover:from-cyan-400 hover:to-blue-500 text-slate-950 font-bold font-mono text-xs shadow-lg shadow-cyan-500/20 transition"
                >
      ...
      {selectedIssueForWorkOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
          <div className="glass-panel p-6 max-w-2xl w-full border-cyan-500/40 bg-slate-950 shadow-2xl rounded-3xl space-y-5 animate-in fade-in zoom-in duration-200">
            {/* Modal Header */}
            <div className="flex items-center justify-between border-b border-slate-800 pb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-lg font-black text-white">Pune Municipal Corporation (PMC)</h3>
                  <div className="text-xs font-mono text-cyan-400">Road Maintenance & Engineering Division • Work Order Dispatch</div>
                </div>
              </div>
              <button 
            ...
            <div className="bg-slate-900/90 rounded-2xl p-4 border border-slate-800 space-y-3 font-mono text-xs">
              <div className="flex items-center justify-between border-b border-slate-800 pb-2">
                <div>
                  <span className="text-slate-400 text-[10px] block">WORK ORDER NUMBER:</span>
                  <span className="text-white font-bold text-sm">PMC-WO-2026-{selectedIssueForWorkOrder.id.replace("RD-", "")}89</span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 pt-1">
                ...
                <div>
                  <span className="text-slate-400 text-[10px] block">MUNICIPAL WARD:</span>
                  <span className="text-cyan-300 font-bold">{selectedIssueForWorkOrder.ward || "Pune Central Ward"}</span>
                </div>
                <div>
                  <span className="text-slate-400 text-[10px] block">GEO-COORDINATES:</span>
                  <span className="text-slate-200">{selectedIssueForWorkOrder.latitude}° N, {selectedIssueForWorkOrder.longitude}° E</span>
                </div>
                <div>
                  <span className="text-slate-400 text-[10px] block">EDGE AI CORROBORATION:</span>
              ...
              <div className="pt-2 border-t border-slate-800">
                <span className="text-slate-400 text-[10px] block mb-1">RECOMMENDED REPAIR DIRECTIVE:</span>
                <p className="text-slate-200 font-sans text-xs bg-slate-950 p-3 rounded-xl border border-slate-800 leading-relaxed">
            ...
            <div className="flex items-center justify-between pt-2">
              <button
                ...
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white font-mono text-xs transition"
              >
              ...
              <button
                ...
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-slate-950 font-black font-mono text-xs shadow-lg shadow-emerald-500/20 transition"
              >
                <Send className="w-4 h-4" /> Dispatch to PWD Road Maintenance Crew
              </button>
            </div>
          </div>
        </div>
